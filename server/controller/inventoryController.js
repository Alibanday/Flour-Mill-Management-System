  ...
  try {
    ...
    if (req.user?.role === 'Warehouse Manager') {
      if (req.user.assignedWarehouse) {
        ...
        filter.$and = [
          { warehouse: req.user.assignedWarehouse }
        ];
      } else {
        // Fallback to legacy logic or return empty
        try {
...
export const getInventorySummary = async (req, res) => {
  try {
    ...
    if (req.user?.role === 'Warehouse Manager') {
      if (req.user.assignedWarehouse) {
        warehouseFilter.warehouse = req.user.assignedWarehouse;
      } else {
        try {
          const Warehouse = (await import('../model/wareHouse.js')).default;
          const managedWarehouses = await Warehouse.find({ manager: req.user._id }).select('_id');
...
export const getInventoryByWarehouse = async (req, res) => {
  try {
    ...
    if (req.user?.role === 'Warehouse Manager') {
      ...
      if (assignedId && assignedId !== warehouseId) {

...
const AccountDetail = () => {
  ...
  if (!account) {
    return (
      <div className="fixed inset-0 bg-gray-50 flex items-center justify-center">
        <p className="text-gray-600 text-lg">Account not found</p>
      </div>
    );
  }
          ...
          <div className="p-6 md:p-8 h-full">
            <div className="flex flex-col h-full">
              <div className="flex justify-between items-center mb-8">
                <h1 className="text-2xl md:text-3xl font-bold text-gray-800">Account Detail</h1>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-grow">
                <DetailCard title="Account ID" value={account.accountId} />
                <DetailCard title="Account Type" value={account.accountType} />
                <DetailCard title="Account Name" value={account.accountName} />
                <DetailCard title="Phone Number" value={account.phoneNumber} />
                <DetailCard title="WhatsApp Number" value={account.whatsappNumber} />
                <DetailCard title="Credit Limit" value={`Rs. ${account.creditLimit}`} />
                <DetailCard title="Address" value={account.address} />
                <DetailCard title="Created At" value={new Date(account.createdAt).toLocaleString()} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl overflow-hidden w-full max-w-md">
            <div className="p-6">
              ...
              <p className="text-gray-600 text-center mb-6">Are you sure you want to delete this account? This action cannot be undone.</p>

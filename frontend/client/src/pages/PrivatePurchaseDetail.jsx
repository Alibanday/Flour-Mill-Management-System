...
import { FaArrowLeft, FaPrint, FaEdit, FaTrash, FaWallet, FaMoneyBillWave, FaCalendarAlt, FaInfoCircle } from "react-icons/fa";
import { toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";

export default function PrivatePurchaseDetail() {
  const { id } = useParams();
  ...
  const handleEdit = () => {
    navigate(`/private-purchases/edit/${id}`);
  };
  ...
  const handleDelete = async () => {
    if (window.confirm("Are you sure you want to delete this purchase record?")) {
      try {
        ...
        navigate("/private-purchases", { state: { message: "Purchase record deleted successfully" } });
        toast.success("Purchase record deleted successfully");
      } catch (err) {
        console.error("Failed to delete purchase:", err);
        setError(err.response?.data?.message || "Failed to delete purchase record");
      ...
      <div className="border border-gray-200 rounded-lg overflow-hidden print:border-0">
        ...
        <div className="bg-blue-600 text-white p-4 flex justify-between items-center">
          <div className="flex items-center">
            ...
            <h2 className="text-xl font-bold">Private Wheat Purchase</h2>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className={`p-4 rounded-md ${purchase.remainingAmount > 0 ? 'bg-yellow-50' : 'bg-green-50'}`}>
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className={`font-medium ${purchase.remainingAmount > 0 ? 'text-yellow-700' : 'text-green-700'}`}>

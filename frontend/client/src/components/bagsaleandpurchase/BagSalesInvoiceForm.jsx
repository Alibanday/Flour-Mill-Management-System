...
export default function BagSalesInvoiceForm({ onInvoiceCreated, onCancel }) {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        // Filter only bag items for dropdown
  ...
  useEffect(() => {
    ...
    if (itemCategory !== "bags") return;
    if (!itemInput.weight) return;
  ...
  useEffect(() => {
    const fetchCreditInfo = async () => {
      ...
      try {
        const token = localStorage.getItem("token");
        // Fetch account
        const accRes = await axios.get(`http://localhost:8000/api/accounts/${form.customerAccount}`, { headers: { Authorization: `Bearer ${token}` } });
        const creditLimit = accRes.data.creditLimit || 0;
        // Fetch invoices for this account
        const invRes = await axios.get(`http://localhost:8000/api/invoice/account/${form.customerAccount}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleSave = async (printAfter=false) => {
    ...
    // Credit limit validation - check if remaining amount + outstanding exceeds credit limit
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md">
      ...
      {/* Move Initial Payment field here, above the summary */}
